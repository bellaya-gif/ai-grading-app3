...
st.caption(
    ...
    "채점 결과와 보완할 점을 먼저 확인하고, 같은 화면에서 모범 답안을 열어 볼 수 있습니다."
)
...
def show_model_answer(title, answer):
    """학생이 채점 결과를 확인한 뒤 같은 화면에서 모범 답안을 열어 볼 수 있게 한다."""
    with st.expander(f"📖 {title} — 모범 답안 확인하기", expanded=False):
        st.markdown(answer)
        st.caption(
            "※ 모범 답안과 표현이 조금 다르더라도 문법적 의미가 정확하면 "
            "정답으로 인정될 수 있습니다."
        )
...
- ‘선물했다’는 **세 자리 서술어**임.
- ‘지훈이가’는 주어, ‘예쁜 꽃다발을’은 목적어, ‘유나에게’는 **필수적 부사어**임.
- ‘유나에게’는 선물을 **받는 대상**을 나타내며, ‘선물하다’라는 서술어가 의미상 필수적으로 요구하는 성분이므로 필수적 부사어임.
"""
        )
...
- (가) ‘교사가’ → **보어**
- (나) ‘물로’ → **필수적 부사어**
- 두 성분 모두 서술어가 의미상 요구하는 성분이지만, (가)의 ‘교사가’에는 **보격 조사 ‘이/가’**가 결합하여 보어가 되고, (나)의 ‘물로’에는 **부사격 조사 ‘로’**가 결합하여 필수적 부사어가 됨.
- 따라서 문장에서 서술어가 요구하는 성분인지와 함께 **조사의 종류와 문장 구조**를 살펴보아야 함.
"""
        )
...
- (가)는 ‘동생이 케이크를 만들었다’에서 관형절 내부의 **목적어 ‘케이크를’**이 생략되고, 그 성분이 뒤의 명사 ‘케이크’와 대응하는 구조임.
- (나)는 ‘우리가 우승했다’라는 **절 전체가 ‘소문’의 내용**을 나타내므로, 관형절 내부에 뒤의 명사와 대응하여 생략된 문장 성분이 없는 구조임.
"""
        )
...
- 연결 어미: **‘-아서/-어서’**(‘와서’)
- 앞 절의 ‘비가 많이 온 것’이 뒤 절의 ‘경기가 취소된 것’의 **원인·이유**가 됨.
- 따라서 두 절은 **종속적인 의미 관계**로 이어진 이어진 문장임.
"""
        )
...
- (가) **‘우리가 시험을 잘 치르기’** → **명사절** / 전체 문장에서 **목적어 역할**
- (나) **‘잎이 푸르다’** → **서술절** / 전체 문장에서 **주어인 ‘그 나무는’의 상태를 설명하는 서술어 역할**
- (다) **‘함께 가자’** → **인용절** / 전체 문장에서 **‘말했다’를 수식하는 부사어 역할**
"""
        )
